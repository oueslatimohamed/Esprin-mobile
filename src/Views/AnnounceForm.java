...
public class AnnounceForm extends SideMenuBaseForm{
    public AnnounceForm(Resources res) {
        super(BoxLayout.y());
        ...
        add(new Label("Announces", "TodayTitle"));

        FontImage arrowDown = FontImage.createMaterial(FontImage.MATERIAL_KEYBOARD_ARROW_DOWN, "Label", 3);
        Container by = new Container();
        ArrayList<Annoucement> annoucements = AnnounceServices.getInstance().getAllann();
        for(int i=0;i<annoucements.size();i++){
            by = BoxLayout.encloseY(
                    AnnounceCell(annoucements.get(i),res)
            );
            add(by);
        }
    ...
    public Container AnnounceCell(Annoucement e,Resources theme){

...
public class OfferForm extends SideMenuBaseForm{
    public OfferForm(Resources res) {
        super(BoxLayout.y());
        ...
        add(new Label("Offers", "TodayTitle"));

        FontImage arrowDown = FontImage.createMaterial(FontImage.MATERIAL_KEYBOARD_ARROW_DOWN, "Label", 3);
        Container by = new Container();
        ArrayList<Offre> offres = OfferServices.getInstance().getAllevents();
        for(int i=0;i<offres.size();i++){
            by = BoxLayout.encloseY(
                    offerCell(offres.get(i),res)
            );
            add(by);
        }
    ...
    public Container offerCell(Offre e, Resources theme){
